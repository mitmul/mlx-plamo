...
class LoRALinear(nn.Module):  # type: ignore
    @staticmethod
    def from_linear(linear: nn.Linear, rank: int = 8) -> Any:
    ...
    def to_linear(self) -> nn.QuantizedLinear:
        linear = self.linear
        bias = "bias" in linear
...
class PlamoModel(nn.Module):  # type: ignore
    ...
    def __call__(
        ...
        if cache is None:
            ...
            past_key_values_length = cache[0][0].shape[2] if cache[0] is not None else 0
